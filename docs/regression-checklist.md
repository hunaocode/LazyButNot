...
若本次改动涉及设置页、统计页、首页卡片、目标列表卡片或主题分组，还必须额外检查：
- 卡片到页面左右的距离，是否仍与基准页面一致
- 卡片内部内容的内边距是否保持原值，没有因为改背景承载层而消失
- 模块与模块之间的垂直间距是否仍存在
- 设置页外观主题中的各色系分组之间，是否仍存在单独留白
- 一个业务模块是否仍是一张卡，没有被拆成多张行卡
- 若需求是“只改配色”，则 `listStyle`、`listRowInsets`、`spacing`、卡片层级不应发生变化
- 若卡片背景从 `listRowBackground` 改到内容 `.background`，要确认外边距和内边距都仍然成立
- 若卡片背景仍在 `listRowBackground`，要确认没有把模块之间的留白吃掉
